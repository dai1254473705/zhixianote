...
- **一句话定位**：本地优先的办公资料笔记本。
- **核心卖点**：
  - 所有数据保存在本地，不上传、不注册，数据在你的设备上；
  - PDF 预览与文字提取、图片压缩，办公常用功能直接内建；
  - 待办、日程、密码、备忘录，在一个应用里统一管理；
  - 导出 .zxnbackup 备份包，换电脑一键恢复全部数据；
  - 敏感笔记和密码支持本地加密，主密码保护。
- 官网必须在首屏清晰回答三个问题：
  1. 这是什么工具；
  2. 适合谁用（办公人员、资料整理、写笔记等）；
  3. 和普通云笔记相比的差异点（本地优先、免注册、数据在手里、备份恢复）。
...
- **使用场景**：
  - 「会议资料整理」：会前收资料，会中记纪要，会后提待办；
  - 「项目资料归档」：PDF、截图、链接、密码、日程集中管理；
  - 「换电脑备份恢复」：导出 `.zxnbackup`，导入后恢复功能百宝箱数据；
  - 每个场景用 2～3 行文字+一张界面截图说明。
- **推广案例**：
  - 「小红书」：围绕办公资料分散、PDF/截图/会议纪要不好找来写；
  - 「B站」：用 3 分钟录屏展示从 PDF 整理到待办提取、备份导出的完整流程；
  - 「社群」：强调本地优先、无需注册、换电脑可用 `.zxnbackup` 恢复；
  - 推广文案只讲已实现能力，不承诺未来功能。
- **简单上手步骤**：
  - 第一步：下载并安装应用；
  - 第二步：选择或创建本地笔记仓库；
